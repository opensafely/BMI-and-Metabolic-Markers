...
explanatory_vars <- c("precovid_bmi_category", 
                      "region", 
                      "hypertension",  
                      "diabetes_t1",           
                      "diabetes_t2",
...
models_trajectory_change <- explanatory_vars %>%       # begin with variables of interest
  str_c("trajectory_change ~ sex + age_group_2 + imd + eth_group_16 +", .) %>%         # combine each variable into formula ("outcome ~ variable of interest")
  
  # iterate through each sexage_adjusted formula
  map(                               
...
# develop a vector of explanatory variables
explanatory_vars_hypertension <- c("precovid_bmi_category", 
                      "region", 
                      "diabetes_t1",           
                      "diabetes_t2",
                      "chronic_cardiac", 
...
# develop a vector of explanatory variables
explanatory_vars_diabetes <- c("precovid_bmi_category", 
                      "region", 
                      "hypertension",           
                      "chronic_cardiac", 
                      "learning_disability",     
...
write.csv (models_trajectory_change, here::here ("output/data","sexageimdethnic_adjusted_lowbmi_excluded_bmi_trajectory_change.csv"))
write.csv (models_trajectory_change_hypertension, here::here ("output/data","sexageimdethnic_adjusted_lowbmi_excluded_bmi_trajectory_change_hypertension.csv"))
write.csv (models_trajectory_change_T2DM, here::here ("output/data","sexageimdethnic_adjusted_lowbmi_excluded_bmi_trajectory_change_T2DM.csv"))
write.csv (models_trajectory_change_T1DM, here::here ("output/data","sexageimdethnic_adjusted_lowbmi_excluded_bmi_trajectory_change_T1DM.csv"))
