...
mean_population = mean(BMI_trajectories$trajectory_change)
sd_population = sd(BMI_trajectories$trajectory_change)
...
trajectory_all <-  data.frame(t(sapply(quantile_all,c))) %>%  # change list of qauntiles to a data frame
  ...
  dplyr::mutate(mean = mean_population) %>%
  dplyr::mutate(stand_dev = sd_population) %>%
...
trajectory_all <- trajectory_all %>%
  ...
  dplyr::mutate(group = "all") %>%
  dplyr::mutate(variable = "all") %>%
  dplyr::select('variable', 'group', 'N', 'N_population', '25th', '50th', '75th', 'mean', 'stand_dev') 
...
mean_trajectory_age_group_2 <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="age_group_2"]
sd_trajectory_age_group_2 <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="age_group_2"]
...
trajectory_age_group_2 <- N_trajectory_age_group_2 %>%
  ...
  dplyr::left_join(mean_trajectory_age_group_2, by = "age_group_2") %>%
  dplyr::left_join(sd_trajectory_age_group_2, by = "age_group_2") %>%
  dplyr::select(age_group_2, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_sex <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="sex"]
sd_trajectory_sex <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="sex"]
...
trajectory_sex <- N_trajectory_sex %>%
  ...
  dplyr::left_join(mean_trajectory_sex, by = "sex") %>%
  dplyr::left_join(sd_trajectory_sex, by = "sex") %>%
  dplyr::select(sex, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_region <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="region"]
sd_trajectory_region <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="region"]
...
trajectory_region <- N_trajectory_region %>%
  ...
  dplyr::left_join(mean_trajectory_region, by = "region") %>%
  dplyr::left_join(sd_trajectory_region, by = "region") %>%
  dplyr::select(region, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_imd <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="imd"]
sd_trajectory_imd <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="imd"]
...
trajectory_imd <- N_trajectory_imd %>%
  ...
  dplyr::left_join(mean_trajectory_imd, by = "imd") %>%
  dplyr::left_join(sd_trajectory_imd, by = "imd") %>%
  dplyr::select(imd, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_ethnic_no_miss <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="ethnic_no_miss"]
sd_trajectory_ethnic_no_miss <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="ethnic_no_miss"]
...
trajectory_ethnic_no_miss <- N_trajectory_ethnic_no_miss %>%
  ...
  dplyr::left_join(mean_trajectory_ethnic_no_miss, by = "ethnic_no_miss") %>%
  dplyr::left_join(sd_trajectory_ethnic_no_miss, by = "ethnic_no_miss") %>%
  dplyr::select(ethnic_no_miss, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_eth_group_16 <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="eth_group_16"]
sd_trajectory_eth_group_16 <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="eth_group_16"]
...
trajectory_eth_group_16 <- N_trajectory_eth_group_16 %>%
  ...
  dplyr::left_join(mean_trajectory_eth_group_16, by = "eth_group_16") %>%
  dplyr::left_join(sd_trajectory_eth_group_16, by = "eth_group_16") %>%
  dplyr::select(eth_group_16, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_learning_disability <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="learning_disability"]
sd_trajectory_learning_disability <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="learning_disability"]
...
trajectory_learning_disability <- N_trajectory_learning_disability %>%
  ...
  dplyr::left_join(mean_trajectory_learning_disability, by = "learning_disability") %>%
  dplyr::left_join(sd_trajectory_learning_disability, by = "learning_disability") %>%
  dplyr::select(learning_disability, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_depression <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="depression"]
sd_trajectory_depression <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="depression"]
...
trajectory_depression <- N_trajectory_depression %>%
  ...
  dplyr::left_join(mean_trajectory_depression, by = "depression") %>%
  dplyr::left_join(sd_trajectory_depression, by = "depression") %>%
  dplyr::select(depression, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_dementia <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="dementia"]
sd_trajectory_dementia <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="dementia"]
...
trajectory_dementia <- N_trajectory_dementia %>%
  ...
  dplyr::left_join(mean_trajectory_dementia, by = "dementia") %>%
  dplyr::left_join(sd_trajectory_dementia, by = "dementia") %>%
  dplyr::select(dementia, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_psychosis_schiz_bipolar <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="psychosis_schiz_bipolar"]
sd_trajectory_psychosis_schiz_bipolar <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="psychosis_schiz_bipolar"]
...
trajectory_psychosis_schiz_bipolar <- N_trajectory_psychosis_schiz_bipolar %>%
  ...
  dplyr::left_join(mean_trajectory_psychosis_schiz_bipolar, by = "psychosis_schiz_bipolar") %>%
  dplyr::left_join(sd_trajectory_psychosis_schiz_bipolar, by = "psychosis_schiz_bipolar") %>%
  dplyr::select(psychosis_schiz_bipolar, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_diabetes_t1 <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="diabetes_t1"]
sd_trajectory_diabetes_t1 <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="diabetes_t1"]
...
trajectory_diabetes_t1 <- N_trajectory_diabetes_t1 %>%
  ...
  dplyr::left_join(mean_trajectory_diabetes_t1, by = "diabetes_t1") %>%
  dplyr::left_join(sd_trajectory_diabetes_t1, by = "diabetes_t1") %>%
  dplyr::select(diabetes_t1, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_diabetes_t2 <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="diabetes_t2"]
sd_trajectory_diabetes_t2 <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="diabetes_t2"]
...
trajectory_diabetes_t2 <- N_trajectory_diabetes_t2 %>%
  ...
  dplyr::left_join(mean_trajectory_diabetes_t2, by = "diabetes_t2") %>%
  dplyr::left_join(sd_trajectory_diabetes_t2, by = "diabetes_t2") %>%
  dplyr::select(diabetes_t2, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_asthma <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="asthma"]
sd_trajectory_asthma <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="asthma"]
...
trajectory_asthma <- N_trajectory_asthma %>%
  ...
  dplyr::left_join(mean_trajectory_asthma, by = "asthma") %>%
  dplyr::left_join(sd_trajectory_asthma, by = "asthma") %>%
  dplyr::select(asthma, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_COPD <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="COPD"]
sd_trajectory_COPD <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="COPD"]
...
trajectory_COPD <- N_trajectory_COPD %>%
  ...
  dplyr::left_join(mean_trajectory_COPD, by = "COPD") %>%
  dplyr::left_join(sd_trajectory_COPD, by = "COPD") %>%
  dplyr::select(COPD, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_stroke_and_TIA <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="stroke_and_TIA"]
sd_trajectory_stroke_and_TIA <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="stroke_and_TIA"]
...
trajectory_stroke_and_TIA <- N_trajectory_stroke_and_TIA %>%
  ...
  dplyr::left_join(mean_trajectory_stroke_and_TIA, by = "stroke_and_TIA") %>%
  dplyr::left_join(sd_trajectory_stroke_and_TIA, by = "stroke_and_TIA") %>%
  dplyr::select(stroke_and_TIA, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_chronic_cardiac <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="chronic_cardiac"]
sd_trajectory_chronic_cardiac <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="chronic_cardiac"]
...
trajectory_chronic_cardiac <- N_trajectory_chronic_cardiac %>%
  ...
  dplyr::left_join(mean_trajectory_chronic_cardiac, by = "chronic_cardiac") %>%
  dplyr::left_join(sd_trajectory_chronic_cardiac, by = "chronic_cardiac") %>%
  dplyr::select(chronic_cardiac, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_hypertension <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="hypertension"]
sd_trajectory_hypertension <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="hypertension"]
...
trajectory_hypertension <- N_trajectory_hypertension %>%
  ...
  dplyr::left_join(mean_trajectory_hypertension, by = "hypertension") %>%
  dplyr::left_join(sd_trajectory_hypertension, by = "hypertension") %>%
  dplyr::select(hypertension, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_all_cancer <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="all_cancer"]
sd_trajectory_all_cancer <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="all_cancer"]
...
trajectory_all_cancer <- N_trajectory_all_cancer %>%
  ...
  dplyr::left_join(mean_trajectory_all_cancer, by = "all_cancer") %>%
  dplyr::left_join(sd_trajectory_all_cancer, by = "all_cancer") %>%
  dplyr::select(all_cancer, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_smoking_status <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="smoking_status"]
sd_trajectory_smoking_status <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="smoking_status"]
...
trajectory_smoking_status <- N_trajectory_smoking_status %>%
  ...
  dplyr::left_join(mean_trajectory_smoking_status, by = "smoking_status") %>%
  dplyr::left_join(sd_trajectory_smoking_status, by = "smoking_status") %>%
  dplyr::select(smoking_status, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
mean_trajectory_precovid_bmi_category <- BMI_trajectories_DT[, .( "mean" = mean(trajectory_change)), by="precovid_bmi_category"]
sd_trajectory_precovid_bmi_category <- BMI_trajectories_DT[, .( "stand_dev" = sd(trajectory_change)), by="precovid_bmi_category"]
...
trajectory_precovid_bmi_category <- N_trajectory_precovid_bmi_category %>%
  ...
  dplyr::left_join(mean_trajectory_precovid_bmi_category, by = "precovid_bmi_category") %>%
  dplyr::left_join(sd_trajectory_precovid_bmi_category, by = "precovid_bmi_category") %>%
  dplyr::select(precovid_bmi_category, "N", N_population, "25th", '50th', "75th", "mean", "stand_dev") %>%
...
trajectory_complete <- as.data.frame(trajectory_complete) %>%
  dplyr::mutate(N = plyr::round_any(trajectory_complete$N, 5)) %>% 
  dplyr::mutate(N_population = plyr::round_any(trajectory_complete$N_population, 5)) 

write.csv (trajectory_complete, here::here ("output/data","trajectory_change_median_IQR_mean.csv"))