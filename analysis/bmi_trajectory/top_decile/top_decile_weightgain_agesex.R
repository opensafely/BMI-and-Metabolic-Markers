...
models_precov_rapidinc_bmi_univar <- explanatory_vars %>%       # begin with variables of interest
  str_c("weightgain_90th ~ sex + age_group_2 +", .) %>%         # combine each variable into formula ("outcome ~ variable of interest")
...
### Write outputs
models_agesex <- models_precov_rapidinc_bmi_univar %>% 
  bind_rows(models_postcov_rapidinc_bmi_univar)


write_csv (models_agesex, here::here ("output/data","weightgain_90th_agesex_adj.csv"))
