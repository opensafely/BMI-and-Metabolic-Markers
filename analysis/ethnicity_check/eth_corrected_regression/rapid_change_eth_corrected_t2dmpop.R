...
models_ageseximd_precov <- models_precov_rapidinc_bmi_age %>%
...
models_precov_ethnicity <- explanatory_vars_2 %>%       # begin with variables of interest
...
models_precov_ethnicity <- models_precov_ethnicity %>%
  dplyr::mutate(stage = "precovid", .before = 1) 
...
models_ageseximd_postcov <- models_postcov_rapidinc_bmi_age %>%
...
# eth adjusted
models_postcov_ethnicity <- explanatory_vars %>%       # begin with variables of interest
  str_c("rapid_bmi_change ~ eth_16_corrected + ", .) %>%         # combine each variable into formula ("outcome ~ variable of interest")
...
models_postcov_ethnicity <- models_postcov_ethnicity %>%
  dplyr::mutate(stage = "postcovid", .before = 1) 
...
models_ageseximd <- models_ageseximd_precov %>%
  bind_rows(models_ageseximd_postcov)

models_ethnicity <- models_precov_ethnicity %>% 
  bind_rows(models_postcov_ethnicity)
...
write_csv (models_ethnicity, here::here ("output/data","rapid_bmi_change_cancerandlowbmi_removed_eth_corrected_eth_adj_t2dm.csv"))
write_csv (models_ageseximd, here::here ("output/data","rapid_bmi_change_cancerandlowbmi_removed_eth_corrected_ageseximd_adj_t2dm.csv"))

write_csv (models_univariate, here::here ("output/data","rapid_bmi_change_cancerandlowbmi_removed_eth_corrected_univariate_t2dm.csv"))
write_csv (demog, here::here ("output/data","rapid_bmi_change_cancerandlowbmi_removed_eth_corrected_popcharac_t2dm.csv"))