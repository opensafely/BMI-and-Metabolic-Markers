...
models_age_group_2 <- eth_group_16 %>%
  bind_rows(imd) %>%
  bind_rows(hypertension) %>%
  bind_rows(chronic_cardiac) %>%
  bind_rows(learning_disability) %>%
...
write_csv (complete_male, here::here ("output/data","CC_delta_pandemic_models_1_t2d_male.csv"))
write_csv (complete_female, here::here ("output/data","CC_delta_pandemic_models_1_t2d_female.csv"))
