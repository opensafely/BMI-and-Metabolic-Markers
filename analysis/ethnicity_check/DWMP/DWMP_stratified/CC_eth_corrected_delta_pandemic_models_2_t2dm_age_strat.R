...
hypertension <- DT[, glm(rapid_bmi_change ~ sex + eth_collapsed + imd + hypertension, family = "binomial")] %>% 
  tidy(exponentiate = TRUE, conf.int = TRUE) %>%        # exponentiate and produce CIs
  mutate(across(where(is.numeric), round, digits = 5)) 
...
diabetes_med <- DT[, glm(rapid_bmi_change ~ sex + eth_collapsed + imd + diabetes_med, family = "binomial")] %>% 
  tidy(exponentiate = TRUE, conf.int = TRUE) %>%        # exponentiate and produce CIs
  mutate(across(where(is.numeric), round, digits = 5)) 
...
write_csv (complete_18_39, here::here ("output/data","CC_delta_pandemic_models_2_t2d_18_39.csv"))
write_csv (complete_40_59, here::here ("output/data","CC_delta_pandemic_models_2_t2d_40_59.csv"))
write_csv (complete_60_79, here::here ("output/data","CC_delta_pandemic_models_2_t2d_60_79.csv"))