## Author: M Samuel 
## Date: 5th April 
##This file looks at the analysis of predictors of rapid  weight gain in a population limited to those with T2D
...
#prepandemic <- CC_stratified_analysis_delta_data_prepandemic
...
prepandemic_models <- region %>%
  ...
  bind_rows(stroke_and_TIA) %>%
  bind_rows(precovid_bmi_category) %>% 
  dplyr::mutate(stage = "prepandemic")



complete <- prepandemic_models %>% 
bind_rows(pandemic_models)



write_csv (complete, here::here ("output/data","CC_delta_models_diabetes_meds.csv"))
