...
## flag diabetes meds
study_data <- study_data %>% mutate_at(vars("insulin_meds", "oad_meds"), ~replace_na(.,0))


study_data <- study_data %>% dplyr::mutate(diabetes_med = case_when(
...
diabetes_med <-function_1(study_data,  diabetes_med)
...
diabetes_med <-function_2(study_data,  diabetes_med)
...
diabetes_med <-function_3(study_data,  diabetes_med)
...
diabetes_med <-function_4(study_data,  diabetes_med)
...
write_csv (complete_data, here::here ("output/data","CC_study_population_characteristics_t2d.csv"))