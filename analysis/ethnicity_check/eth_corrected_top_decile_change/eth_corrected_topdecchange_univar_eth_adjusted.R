...
explanatory_vars_2 <- c("age_group_2", "sex", "imd",
                      "region",                
                      "hypertension",
                      "diabetes_t1", 
                      "diabetes_t2",
...
models_eth <- explanatory_vars_2 %>%       # begin with variables of interest
  ...
  # round all numeric columns
  mutate(across(where(is.numeric), round, digits = 4)) %>% 
  mutate( model = "ethnicity_adjusted")



models <- models_univar %>%
    bind_rows(models_eth)
...
write_csv (models, here::here ("output/data","change_90th_univariate_eth_adjusted_lowbmiexc_eth_corrected.csv"))

write_csv (quantiles, here::here ("output/data","change_deciles_lowbmiexc_eth_corrected.csv"))
