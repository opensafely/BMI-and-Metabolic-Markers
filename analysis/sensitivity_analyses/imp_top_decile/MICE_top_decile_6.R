##  This R script predicts imputed and sample predictors of top decile change
...
##
ethnic <- glm(top_decile_change ~ age_group_2 + sex +   eth_group_16, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_ethnic <- with(BMI_imp_long_mids,
                   glm(top_decile_change ~ age_group_2 + sex +   eth_group_16, family=binomial))
...
##
imd <- glm(top_decile_change ~ age_group_2 + sex + imd, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_imd <- with(BMI_imp_long_mids,
                glm(top_decile_change ~ age_group_2 + sex + imd, family=binomial))
...
##
region  <- glm(top_decile_change ~ age_group_2 + sex + region , family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_region  <- with(BMI_imp_long_mids,
                    glm(top_decile_change ~ age_group_2 + sex + region , family=binomial))
...
hypertension <- glm(top_decile_change ~ age_group_2 + sex + hypertension, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_hypertension <- with(BMI_imp_long_mids,
                         glm(top_decile_change ~ age_group_2 + sex + hypertension, family=binomial))
...
##
diabetes_t1 <- glm(top_decile_change ~ age_group_2 + sex + diabetes_t1, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_diabetes_t1 <- with(BMI_imp_long_mids,
                        glm(top_decile_change ~ age_group_2 + sex + diabetes_t1, family=binomial))
...
diabetes_t2 <- glm(top_decile_change ~ age_group_2 + sex + diabetes_t2, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_diabetes_t2 <- with(BMI_imp_long_mids,
                        glm(top_decile_change ~ age_group_2 + sex + diabetes_t2, family=binomial))
...
chronic_cardiac <- glm(top_decile_change ~ age_group_2 + sex + chronic_cardiac, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_chronic_cardiac <- with(BMI_imp_long_mids,
                            glm(top_decile_change ~ age_group_2 + sex + chronic_cardiac, family=binomial))
...
learning_disability  <- glm(top_decile_change ~ age_group_2 + sex + learning_disability, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_learning_disability  <- with(BMI_imp_long_mids,
                                 glm(top_decile_change ~ age_group_2 + sex + learning_disability , family=binomial))
...
##
psychosis_schiz_bipolar <- glm(top_decile_change ~ age_group_2 + sex + psychosis_schiz_bipolar, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_psychosis_schiz_bipolar <- with(BMI_imp_long_mids,
                                    glm(top_decile_change ~ age_group_2 + sex + psychosis_schiz_bipolar, family=binomial))
...
depression <- glm(top_decile_change ~ age_group_2 + sex + depression, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_depression <- with(BMI_imp_long_mids,
                       glm(top_decile_change ~ age_group_2 + sex + depression, family=binomial))
...
##
COPD <- glm(top_decile_change ~ age_group_2 + sex + COPD, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_COPD <- with(BMI_imp_long_mids,
                 glm(top_decile_change ~ age_group_2 + sex + COPD, family=binomial))
...
##
asthma  <- glm(top_decile_change ~ age_group_2 + sex + asthma, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_asthma  <- with(BMI_imp_long_mids,
                    glm(top_decile_change ~ age_group_2 + sex + asthma, family=binomial))
...
##
dementia  <- glm(top_decile_change ~ age_group_2 + sex + dementia, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_dementia  <- with(BMI_imp_long_mids,
                      glm(top_decile_change ~ age_group_2 + sex + dementia, family=binomial))
...
##
stroke_and_TIA  <- glm(top_decile_change ~ age_group_2 + sex + stroke_and_TIA, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_stroke_and_TIA  <- with(BMI_imp_long_mids,
                            glm(top_decile_change ~ age_group_2 + sex + stroke_and_TIA, family=binomial))
...
smoking_status  <- glm(top_decile_change ~ age_group_2 + sex + smoking_status, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_smoking_status  <- with(BMI_imp_long_mids,
                            glm(top_decile_change ~ age_group_2 + sex + smoking_status, family=binomial))
...
write.csv (models, here::here ("output/data", "age_sex_topdecile_imputation.csv"))