...
BMI_data <- read_feather (here::here ("/Users/miriamsamuel/Documents/Academic GP/Open Safely/Dummy Data", "BMI_trajectory_models_data.feather"))
...
BMI_trajectories <- read_feather (here::here ("/Users/miriamsamuel/Documents/Academic GP/Open Safely/Dummy Data", "BMI_trajectories_final_demog.feather"))
...
BMI_trajectories_save <- BMI_trajectories
...
p_missing_1 <- as.data.frame(sort(p_missing[p_missing > 0], decreasing = TRUE))




describe(BMI_trajectories)
...
## Select only variable that could influence the imputation and used in models
BMI_trajectories <- BMI_trajectories_save %>% 
  dplyr::select(-c("bmi_change1", "bmi_change2", "postcovid_bmi", "time_change1", "time_change2", 
