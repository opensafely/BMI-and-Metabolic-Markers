...
age <- glm(postcovid_rapid ~ sex + age_group_2, family=binomial, data=BMI_trajectories) %>%
  broom::tidy(conf.int = TRUE, exp = TRUE)

imp_age <- with(BMI_imp_long_mids,
                glm(postcovid_rapid ~ sex + age_group_2, family=binomial))
...
##
ethnic <- glm(postcovid_rapid ~ sex +   eth_group_16, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_ethnic <- with(BMI_imp_long_mids,
                   glm(postcovid_rapid ~ sex +   eth_group_16, family=binomial))
...
##
imd <- glm(postcovid_rapid ~ sex + imd, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_imd <- with(BMI_imp_long_mids,
                glm(postcovid_rapid ~ sex + imd, family=binomial))
...
##
region  <- glm(postcovid_rapid ~ sex + region , family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_region  <- with(BMI_imp_long_mids,
                    glm(postcovid_rapid ~ sex + region , family=binomial))
...
hypertension <- glm(postcovid_rapid ~ sex + hypertension, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_hypertension <- with(BMI_imp_long_mids,
                         glm(postcovid_rapid ~ sex + hypertension, family=binomial))
...
##
diabetes_t1 <- glm(postcovid_rapid ~ sex + diabetes_t1, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_diabetes_t1 <- with(BMI_imp_long_mids,
                        glm(postcovid_rapid ~ sex + diabetes_t1, family=binomial))
...
diabetes_t2 <- glm(postcovid_rapid ~ sex + diabetes_t2, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_diabetes_t2 <- with(BMI_imp_long_mids,
                        glm(postcovid_rapid ~ sex + diabetes_t2, family=binomial))
...
chronic_cardiac <- glm(postcovid_rapid ~ sex + chronic_cardiac, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_chronic_cardiac <- with(BMI_imp_long_mids,
                            glm(postcovid_rapid ~ sex + chronic_cardiac, family=binomial))
...
learning_disability  <- glm(postcovid_rapid ~ sex + learning_disability, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_learning_disability  <- with(BMI_imp_long_mids,
                                 glm(postcovid_rapid ~ sex + learning_disability , family=binomial))
...
##
psychosis_schiz_bipolar <- glm(postcovid_rapid ~ sex + psychosis_schiz_bipolar, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_psychosis_schiz_bipolar <- with(BMI_imp_long_mids,
                                    glm(postcovid_rapid ~ sex + psychosis_schiz_bipolar, family=binomial))
...
depression <- glm(postcovid_rapid ~ sex + depression, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_depression <- with(BMI_imp_long_mids,
                       glm(postcovid_rapid ~ sex + depression, family=binomial))
...
##
COPD <- glm(postcovid_rapid ~ sex + COPD, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_COPD <- with(BMI_imp_long_mids,
                 glm(postcovid_rapid ~ sex + COPD, family=binomial))
...
##
asthma  <- glm(postcovid_rapid ~ sex + asthma, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_asthma  <- with(BMI_imp_long_mids,
                    glm(postcovid_rapid ~ sex + asthma, family=binomial))
...
##
dementia  <- glm(postcovid_rapid ~ sex + dementia, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_dementia  <- with(BMI_imp_long_mids,
                      glm(postcovid_rapid ~ sex + dementia, family=binomial))
...
##
stroke_and_TIA  <- glm(postcovid_rapid ~ sex + stroke_and_TIA, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_stroke_and_TIA  <- with(BMI_imp_long_mids,
                            glm(postcovid_rapid ~ sex + stroke_and_TIA, family=binomial))
...
smoking_status  <- glm(postcovid_rapid ~ sex + smoking_status, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_smoking_status  <- with(BMI_imp_long_mids,
                            glm(postcovid_rapid ~ sex + smoking_status, family=binomial))
...
write.csv (models, here::here ("output/data", "sex_rapid_weightgain_imputation.csv"))