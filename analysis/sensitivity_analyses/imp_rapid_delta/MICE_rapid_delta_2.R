...
age <- glm(postcovid_rapid ~ age_group_2, family=binomial, data=BMI_trajectories) %>%
  broom::tidy(conf.int = TRUE, exp = TRUE)

imp_age <- with(BMI_imp_long_mids,
                glm(postcovid_rapid ~ age_group_2, family=binomial))
...
##
sex <- glm(postcovid_rapid ~ sex, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_sex <- with(BMI_imp_long_mids,
                glm(postcovid_rapid ~ sex, family=binomial))
...
##
ethnic <- glm(postcovid_rapid ~   eth_group_16, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_ethnic <- with(BMI_imp_long_mids,
                   glm(postcovid_rapid ~   eth_group_16, family=binomial))
...
##
imd <- glm(postcovid_rapid ~ imd, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_imd <- with(BMI_imp_long_mids,
                glm(postcovid_rapid ~ imd, family=binomial))
...
##
region  <- glm(postcovid_rapid ~ region , family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_region  <- with(BMI_imp_long_mids,
                    glm(postcovid_rapid ~ region , family=binomial))
...
hypertension <- glm(postcovid_rapid ~ hypertension, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_hypertension <- with(BMI_imp_long_mids,
                         glm(postcovid_rapid ~ hypertension, family=binomial))
...
##
diabetes_t1 <- glm(postcovid_rapid ~ diabetes_t1, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_diabetes_t1 <- with(BMI_imp_long_mids,
                        glm(postcovid_rapid ~ diabetes_t1, family=binomial))
...
diabetes_t2 <- glm(postcovid_rapid ~ diabetes_t2, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_diabetes_t2 <- with(BMI_imp_long_mids,
                        glm(postcovid_rapid ~ diabetes_t2, family=binomial))
...
chronic_cardiac <- glm(postcovid_rapid ~ chronic_cardiac, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_chronic_cardiac <- with(BMI_imp_long_mids,
                            glm(postcovid_rapid ~ chronic_cardiac, family=binomial))
...
learning_disability  <- glm(postcovid_rapid ~ learning_disability, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_learning_disability  <- with(BMI_imp_long_mids,
                                 glm(postcovid_rapid ~ learning_disability , family=binomial))
...
##
psychosis_schiz_bipolar <- glm(postcovid_rapid ~ psychosis_schiz_bipolar, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_psychosis_schiz_bipolar <- with(BMI_imp_long_mids,
                                    glm(postcovid_rapid ~ psychosis_schiz_bipolar, family=binomial))
...
depression <- glm(postcovid_rapid ~ depression, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_depression <- with(BMI_imp_long_mids,
                       glm(postcovid_rapid ~ depression, family=binomial))
...
##
COPD <- glm(postcovid_rapid ~ COPD, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_COPD <- with(BMI_imp_long_mids,
                 glm(postcovid_rapid ~ COPD, family=binomial))
...
##
asthma  <- glm(postcovid_rapid ~ asthma, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_asthma  <- with(BMI_imp_long_mids,
                    glm(postcovid_rapid ~ asthma, family=binomial))
...
##
dementia  <- glm(postcovid_rapid ~ dementia, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_dementia  <- with(BMI_imp_long_mids,
                      glm(postcovid_rapid ~ dementia, family=binomial))
...
##
stroke_and_TIA  <- glm(postcovid_rapid ~ stroke_and_TIA, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_stroke_and_TIA  <- with(BMI_imp_long_mids,
                            glm(postcovid_rapid ~ stroke_and_TIA, family=binomial))
...
smoking_status  <- glm(postcovid_rapid ~ smoking_status, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_smoking_status  <- with(BMI_imp_long_mids,
                            glm(postcovid_rapid ~ smoking_status, family=binomial))
...
write.csv (univariate_models, here::here ("output/data", "univariate_rapid_weightgain_imputation.csv"))