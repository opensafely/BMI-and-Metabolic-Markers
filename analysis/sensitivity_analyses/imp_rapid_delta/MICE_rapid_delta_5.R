...
age <- glm(postcovid_rapid ~ imd + age_group_2, family=binomial, data=BMI_trajectories) %>%
  broom::tidy(conf.int = TRUE, exp = TRUE)

imp_age <- with(BMI_imp_long_mids,
                glm(postcovid_rapid ~ imd + age_group_2, family=binomial))
...
##
sex <- glm(postcovid_rapid ~ imd + sex, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_sex <- with(BMI_imp_long_mids,
                glm(postcovid_rapid ~ imd + sex, family=binomial))
...
##
ethnic <- glm(postcovid_rapid ~ imd +   eth_group_16, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_ethnic <- with(BMI_imp_long_mids,
                   glm(postcovid_rapid ~ imd +   eth_group_16, family=binomial))
...
##
region  <- glm(postcovid_rapid ~ imd + region , family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_region  <- with(BMI_imp_long_mids,
                    glm(postcovid_rapid ~ imd + region , family=binomial))
...
hypertension <- glm(postcovid_rapid ~ imd + hypertension, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_hypertension <- with(BMI_imp_long_mids,
                         glm(postcovid_rapid ~ imd + hypertension, family=binomial))
...
##
diabetes_t1 <- glm(postcovid_rapid ~ imd + diabetes_t1, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_diabetes_t1 <- with(BMI_imp_long_mids,
                        glm(postcovid_rapid ~ imd + diabetes_t1, family=binomial))
...
diabetes_t2 <- glm(postcovid_rapid ~ imd + diabetes_t2, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_diabetes_t2 <- with(BMI_imp_long_mids,
                        glm(postcovid_rapid ~ imd + diabetes_t2, family=binomial))
...
chronic_cardiac <- glm(postcovid_rapid ~ imd + chronic_cardiac, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_chronic_cardiac <- with(BMI_imp_long_mids,
                            glm(postcovid_rapid ~ imd + chronic_cardiac, family=binomial))
...
learning_disability  <- glm(postcovid_rapid ~ imd + learning_disability, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_learning_disability  <- with(BMI_imp_long_mids,
                                 glm(postcovid_rapid ~ imd + learning_disability , family=binomial))
...
##
psychosis_schiz_bipolar <- glm(postcovid_rapid ~ imd + psychosis_schiz_bipolar, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_psychosis_schiz_bipolar <- with(BMI_imp_long_mids,
                                    glm(postcovid_rapid ~ imd + psychosis_schiz_bipolar, family=binomial))
...
depression <- glm(postcovid_rapid ~ imd + depression, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_depression <- with(BMI_imp_long_mids,
                       glm(postcovid_rapid ~ imd + depression, family=binomial))
...
##
COPD <- glm(postcovid_rapid ~ imd + COPD, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_COPD <- with(BMI_imp_long_mids,
                 glm(postcovid_rapid ~ imd + COPD, family=binomial))
...
##
asthma  <- glm(postcovid_rapid ~ imd + asthma, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_asthma  <- with(BMI_imp_long_mids,
                    glm(postcovid_rapid ~ imd + asthma, family=binomial))
...
##
dementia  <- glm(postcovid_rapid ~ imd + dementia, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_dementia  <- with(BMI_imp_long_mids,
                      glm(postcovid_rapid ~ imd + dementia, family=binomial))
...
##
stroke_and_TIA  <- glm(postcovid_rapid ~ imd + stroke_and_TIA, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_stroke_and_TIA  <- with(BMI_imp_long_mids,
                            glm(postcovid_rapid ~ imd + stroke_and_TIA, family=binomial))
...
smoking_status  <- glm(postcovid_rapid ~ imd + smoking_status, family=binomial, data=BMI_trajectories) %>% 
  broom::tidy(conf.int = TRUE, exp = TRUE)


imp_smoking_status  <- with(BMI_imp_long_mids,
                            glm(postcovid_rapid ~ imd + smoking_status, family=binomial))
...
write.csv (models, here::here ("output/data", "imd_rapid_weightgain_imputation.csv"))