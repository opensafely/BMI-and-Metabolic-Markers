## This script looks at the average weight change in the pandemic period by exposure groups
...
## create summaries to assess for change in mean in sd per subgroup
 #age_group_2
BMI_T2DM_summ_age_group_2 <- BMI_T2DM_DT %>%
  dplyr::group_by(age_group_2)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = age_group_2) %>% 
  dplyr::mutate(variable = 'age_group_2', .before=1) %>% 
...
# region
BMI_T2DM_summ_region <- BMI_T2DM_DT%>%
  dplyr::group_by(region)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = region) %>% 
  dplyr::mutate(variable = 'region', .before=1) %>% 
...
# hypertension
BMI_T2DM_summ_hypertension <- BMI_T2DM_DT%>%
  dplyr::group_by(hypertension)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = hypertension) %>% 
  dplyr::mutate(variable = 'hypertension', .before=1) %>% 
...
# learning_disability
BMI_T2DM_summ_learning_disability <- BMI_T2DM_DT%>%
  dplyr::group_by(learning_disability)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = learning_disability) %>% 
  dplyr::mutate(variable = 'learning_disability', .before=1) %>% 
...
# depression
BMI_T2DM_summ_depression <- BMI_T2DM_DT%>%
  dplyr::group_by(depression)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = depression) %>% 
  dplyr::mutate(variable = 'depression', .before=1) %>% 
...
# psychosis_schiz_bipolar
BMI_T2DM_summ_psychosis_schiz_bipolar <- BMI_T2DM_DT%>%
  dplyr::group_by(psychosis_schiz_bipolar)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = psychosis_schiz_bipolar) %>% 
  dplyr::mutate(variable = 'psychosis_schiz_bipolar', .before=1) %>% 
...
# dementia
BMI_T2DM_summ_dementia <- BMI_T2DM_DT%>%
  dplyr::group_by(dementia)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = dementia) %>% 
  dplyr::mutate(variable = 'dementia', .before=1) %>% 
...
# asthma
BMI_T2DM_summ_asthma <- BMI_T2DM_DT%>%
  dplyr::group_by(asthma)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = asthma) %>% 
  dplyr::mutate(variable = 'asthma', .before=1) %>% 
...
# COPD
BMI_T2DM_summ_COPD <- BMI_T2DM_DT%>%
  dplyr::group_by(COPD)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = COPD) %>% 
  dplyr::mutate(variable = 'COPD', .before=1) %>% 
...
# stroke_and_TIA
BMI_T2DM_summ_stroke_and_TIA <- BMI_T2DM_DT%>%
  dplyr::group_by(stroke_and_TIA)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = stroke_and_TIA) %>% 
  dplyr::mutate(variable = 'stroke_and_TIA', .before=1) %>% 
...
# all_cancer
BMI_T2DM_summ_all_cancer <- BMI_T2DM_DT%>%
  dplyr::group_by(all_cancer)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = all_cancer) %>% 
  dplyr::mutate(variable = 'all_cancer', .before=1) %>% 
...
# smoking_status
BMI_T2DM_summ_smoking_status <- BMI_T2DM_DT%>%
  dplyr::group_by(smoking_status)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = smoking_status) %>% 
  dplyr::mutate(variable = 'smoking_status', .before=1) %>% 
...
# ethnic_no_miss
BMI_T2DM_summ_ethnic_no_miss <- BMI_T2DM_DT%>%
  dplyr::group_by(ethnic_no_miss)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = ethnic_no_miss) %>% 
  dplyr::mutate(variable = 'ethnic_no_miss', .before=1) %>% 
...
# eth_group_16
BMI_T2DM_summ_eth_group_16 <- BMI_T2DM_DT%>%
  dplyr::group_by(eth_group_16)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = eth_group_16) %>% 
  dplyr::mutate(variable = 'eth_group_16', .before=1) %>% 
...
# precovid_BMI_T2DM_category
BMI_T2DM_summ_precovid_BMI_category <- BMI_T2DM_DT%>%
   dplyr::group_by(precovid_bmi_category)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = precovid_bmi_category) %>% 
  dplyr::mutate(variable = 'precovid_bmi_category', .before=1) %>% 
...
# chronic_cardiac
BMI_T2DM_summ_chronic_cardiac <- BMI_T2DM_DT%>%
  dplyr::group_by(chronic_cardiac)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = chronic_cardiac) %>% 
  dplyr::mutate(variable = 'chronic_cardiac', .before=1) %>% 
...
BMI_T2DM_traj_change_summary <- BMI_T2DM_DT_summ %>% 
  bind_rows(BMI_T2DM_summ_age_group_2) %>% 
  bind_rows(BMI_T2DM_summ_sex) %>% 
  bind_rows(BMI_T2DM_summ_ethnic_no_miss) %>% 
  bind_rows(BMI_T2DM_summ_eth_group_16) %>% 
  bind_rows(BMI_T2DM_summ_imd) %>%
  bind_rows(BMI_T2DM_summ_region) %>%
  bind_rows(BMI_T2DM_summ_precovid_BMI_category) %>% 
  bind_rows(BMI_T2DM_summ_hypertension) %>% 
  bind_rows(BMI_T2DM_summ_learning_disability) %>%
  bind_rows(BMI_T2DM_summ_depression) %>% 
  bind_rows(BMI_T2DM_summ_psychosis_schiz_bipolar) %>% 
  bind_rows(BMI_T2DM_summ_chronic_cardiac) %>% 
  bind_rows(BMI_T2DM_summ_COPD) %>%
  bind_rows(BMI_T2DM_summ_asthma)%>%
  bind_rows(BMI_T2DM_summ_dementia) %>% 
  bind_rows(BMI_T2DM_summ_all_cancer) %>% 
  bind_rows(BMI_T2DM_summ_stroke_and_TIA) %>% 
  bind_rows(BMI_T2DM_summ_smoking_status) %>% 
  dplyr::mutate(across(where(is.numeric), round, digits = 4)) %>%
  dplyr::mutate(n = plyr::round_any(n, 5)) 
...
## create summaries to assess for change in mean in sd per subgroup
 #age_group_2
BMI_T1DM_summ_age_group_2 <- BMI_T1DM_DT %>%
  dplyr::group_by(age_group_2)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = age_group_2) %>% 
  dplyr::mutate(variable = 'age_group_2', .before=1) %>% 
...
# region
BMI_T1DM_summ_region <- BMI_T1DM_DT%>%
  dplyr::group_by(region)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = region) %>% 
  dplyr::mutate(variable = 'region', .before=1) %>% 
...
# hypertension
BMI_T1DM_summ_hypertension <- BMI_T1DM_DT%>%
  dplyr::group_by(hypertension)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = hypertension) %>% 
  dplyr::mutate(variable = 'hypertension', .before=1) %>% 
...
# learning_disability
BMI_T1DM_summ_learning_disability <- BMI_T1DM_DT%>%
  dplyr::group_by(learning_disability)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = learning_disability) %>% 
  dplyr::mutate(variable = 'learning_disability', .before=1) %>% 
...
# depression
BMI_T1DM_summ_depression <- BMI_T1DM_DT%>%
  dplyr::group_by(depression)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = depression) %>% 
  dplyr::mutate(variable = 'depression', .before=1) %>% 
...
# psychosis_schiz_bipolar
BMI_T1DM_summ_psychosis_schiz_bipolar <- BMI_T1DM_DT%>%
  dplyr::group_by(psychosis_schiz_bipolar)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = psychosis_schiz_bipolar) %>% 
  dplyr::mutate(variable = 'psychosis_schiz_bipolar', .before=1) %>% 
...
# dementia
BMI_T1DM_summ_dementia <- BMI_T1DM_DT%>%
  dplyr::group_by(dementia)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = dementia) %>% 
  dplyr::mutate(variable = 'dementia', .before=1) %>% 
...
# asthma
BMI_T1DM_summ_asthma <- BMI_T1DM_DT%>%
  dplyr::group_by(asthma)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = asthma) %>% 
  dplyr::mutate(variable = 'asthma', .before=1) %>% 
...
# COPD
BMI_T1DM_summ_COPD <- BMI_T1DM_DT%>%
  dplyr::group_by(COPD)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = COPD) %>% 
  dplyr::mutate(variable = 'COPD', .before=1) %>% 
...
# stroke_and_TIA
BMI_T1DM_summ_stroke_and_TIA <- BMI_T1DM_DT%>%
  dplyr::group_by(stroke_and_TIA)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = stroke_and_TIA) %>% 
  dplyr::mutate(variable = 'stroke_and_TIA', .before=1) %>% 
...
# all_cancer
BMI_T1DM_summ_all_cancer <- BMI_T1DM_DT%>%
  dplyr::group_by(all_cancer)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = all_cancer) %>% 
  dplyr::mutate(variable = 'all_cancer', .before=1) %>% 
...
# smoking_status
BMI_T1DM_summ_smoking_status <- BMI_T1DM_DT%>%
  dplyr::group_by(smoking_status)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = smoking_status) %>% 
  dplyr::mutate(variable = 'smoking_status', .before=1) %>% 
...
# ethnic_no_miss
BMI_T1DM_summ_ethnic_no_miss <- BMI_T1DM_DT%>%
  dplyr::group_by(ethnic_no_miss)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = ethnic_no_miss) %>% 
  dplyr::mutate(variable = 'ethnic_no_miss', .before=1) %>% 
...
# eth_group_16
BMI_T1DM_summ_eth_group_16 <- BMI_T1DM_DT%>%
  dplyr::group_by(eth_group_16)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = eth_group_16) %>% 
  dplyr::mutate(variable = 'eth_group_16', .before=1) %>% 
...
# precovid_BMI_T1DM_category
BMI_T1DM_summ_precovid_BMI_category <- BMI_T1DM_DT%>%
   dplyr::group_by(precovid_bmi_category)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = precovid_bmi_category) %>% 
  dplyr::mutate(variable = 'precovid_bmi_category', .before=1) %>% 
...
# chronic_cardiac
BMI_T1DM_summ_chronic_cardiac <- BMI_T1DM_DT%>%
  dplyr::group_by(chronic_cardiac)%>%
  dplyr::summarise(n = n(),mean = mean(trajectory_change), sd = sd(trajectory_change)) %>% 
  dplyr::rename(group = chronic_cardiac) %>% 
  dplyr::mutate(variable = 'chronic_cardiac', .before=1) %>% 
...
BMI_T1DM_traj_change_summary <- BMI_T1DM_DT_summ %>% 
  bind_rows(BMI_T1DM_summ_age_group_2) %>% 
  bind_rows(BMI_T1DM_summ_sex) %>% 
  bind_rows(BMI_T1DM_summ_ethnic_no_miss) %>% 
  bind_rows(BMI_T1DM_summ_eth_group_16) %>% 
  bind_rows(BMI_T1DM_summ_imd) %>%
  bind_rows(BMI_T1DM_summ_region) %>%
  bind_rows(BMI_T1DM_summ_precovid_BMI_category) %>% 
  bind_rows(BMI_T1DM_summ_hypertension) %>% 
  bind_rows(BMI_T1DM_summ_learning_disability) %>%
  bind_rows(BMI_T1DM_summ_depression) %>% 
  bind_rows(BMI_T1DM_summ_psychosis_schiz_bipolar) %>% 
  bind_rows(BMI_T1DM_summ_chronic_cardiac) %>% 
  bind_rows(BMI_T1DM_summ_COPD) %>%
  bind_rows(BMI_T1DM_summ_asthma)%>%
  bind_rows(BMI_T1DM_summ_dementia) %>% 
  bind_rows(BMI_T1DM_summ_all_cancer) %>% 
  bind_rows(BMI_T1DM_summ_stroke_and_TIA) %>% 
  bind_rows(BMI_T1DM_summ_smoking_status) %>% 
  dplyr::mutate(across(where(is.numeric), round, digits = 4)) %>%
  dplyr::mutate(n = plyr::round_any(n, 5)) 



write.csv (BMI_T1DM_traj_change_summary, here::here ("output/data","mean_bmi_traj_change_cancerandlowbmi_removed_t1dm.csv"))

write.csv (BMI_T2DM_traj_change_summary, here::here ("output/data","mean_bmi_traj_change_cancerandlowbmi_removed_t2dm.csv"))

write.csv (BMI_hypertension_traj_change_summary, here::here ("output/data","mean_bmi_traj_change_cancerandlowbmi_removed_hypertension.csv"))

write.csv (BMI_traj_change_summary, here::here ("output/data","mean_bmi_traj_change_cancerandlowbmi_removed.csv"))
