...
## Univariate analysis of who was obese in 2021
...
obese_age_m <- glm(obese ~ age_group, data=BMI_complete_categories_2021, family=binomial) %>%
...
obese_age_table <- BMI_complete_categories_2021 %>%
  janitor::tabyl (age_group, obese) 

obese_age_table

combined <- obese_age_table %>%           # begin with table of counts
  bind_cols(., obese_age_m) %>%              # combine with the outputs of the regression 
...
models <- explanatory_vars %>%       # begin with variables of interest
  str_c("obese ~ ", .) %>%    ## creates a vector of characters....  each cell contains outcome ~ var (the structure needed for regression formula)
...
univ_tab_base <- explanatory_vars %>% 
  map(.f = 
        ~{BMI_complete_categories_2021 %>%                ## begin with dataset
            group_by(obese) %>%     ## group data set by outcome
            count(.data[[.x]]) %>%    ## produce counts for variable of interest
            pivot_wider(              ## spread to wide format (as in cross-tabulation)
              names_from = obese,   ## column names '0', '1'................
              values_from = n) %>% 
...
univariate_obese_2021 <- univ_tab_base  %>%
  dplyr::rename(obese = '1') %>%
  dplyr::rename(not_obese = '0') %>%
  dplyr::mutate(N_total = obese + not_obese, .after=obese) %>%
  dplyr::mutate(proportion = obese/N_total, .after=obese) %>%
  mutate(across(where(is.numeric), round, digits = 2))
...
####################################################################################
## OUTPUTS
write.csv (univariate_obese_2021, here::here ("output/data","regression_obese_2021.csv"))
