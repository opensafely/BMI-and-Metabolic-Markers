...
BMI_complete_categories_2020 <- BMI_complete_categories_2020 %>%
  ...
  dplyr::mutate(eth_group_16 = as.factor(ethnic_no_miss)) %>%#
  dplyr::mutate(eth_group_16 = fct_relevel(ethnic_no_miss, "White", after = 0))
...
## instructions from R for epi
# Univariate model with age
had_bmi_age_m <- glm(had_bmi ~ age_group, data=BMI_complete_categories_2021, family=binomial) %>%
  broom::tidy(exponentiate = TRUE, conf.int = TRUE) %>%        # exponentiate and produce CIs
  dplyr::mutate(across(where(is.numeric), round, digits = 2))  # round all numeric columns


had_bmi_age_table <- BMI_complete_categories_2021 %>%
  janitor::tabyl (age_group, had_bmi) 
...
## # generate a column of total counts
univariate_had_bmi_2021 <- univ_tab_base  %>%
  dplyr::rename(had_bmi = 'TRUE') %>%
  dplyr::rename(no_bmi = 'FALSE') %>%
  dplyr::mutate(N_total = had_bmi + no_bmi, .after=had_bmi) %>%
  dplyr::mutate(proportion = had_bmi/N_total, .after=had_bmi) %>%
...
write.csv (univariate_had_bmi_2021, here::here ("output/data","regression_had_bmi_2021.csv"))
