...
BMI_all_patients <- BMI_data  %>% 
  ...
  dplyr::slice_head() %>% 
  dplyr::select(-c(DWMP, year, obese, median_bmi))
...
BMI_30 <- BMI_30 %>% 
  dplyr::select("patient_id", "median_bmi", "obese", "year") %>% 
  group_by(patient_id) %>% 
...
all <- BMI_all_patients %>% 
  tabyl(obese) %>% 
  dplyr::rename(group = obese) %>% 
  dplyr::mutate(variable = "obese")
...
write.csv (all, here::here ("output/data","obese_5yr_data_years_2019.csv"))
write.csv (complete_percentages, here::here ("output/data","obese_5yr_data_all_2019.csv"))
write.csv (complete_hypertension, here::here ("output/data","obese_5yr_data_hypertension_2019.csv"))
write.csv (complete_diabetes_t1, here::here ("output/data","obese_5yr_data_T1DM_2019.csv"))
write.csv (complete_diabetes_t2, here::here ("output/data","obese_5yr_data_T2DM_2019.csv"))
