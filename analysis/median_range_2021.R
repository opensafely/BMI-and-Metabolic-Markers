...
N_median_precovid_obese_flag <- BMI_complete_categories_DT [, .(N_population = length(median_bmi)) , by="precovid_obese_flag"]
n_median_precovid_obese_flag <- BMI_complete_categories_DT [, .(N_missing = sum(is.na(median_bmi))) , by="precovid_obese_flag"]
quartile1_median_precovid_obese_flag <- BMI_complete_categories_DT[, .( "25th" = quantile(median_bmi, probs = c(0.25), na.rm = TRUE)), by="precovid_obese_flag"]
median_median_precovid_obese_flag <- BMI_complete_categories_DT[, .( "median" = quantile(median_bmi, probs = c(0.5), na.rm = TRUE)), by="precovid_obese_flag"]
quartile3_median_precovid_obese_flag <- BMI_complete_categories_DT[, .( "75th" = quantile(median_bmi, probs = c(0.75), na.rm = TRUE)), by="precovid_obese_flag"]

KWRS_precovid_obese_flag <- kruskal.test(precovid_obese_flag ~ median_bmi, BMI_complete_categories)
significance_precovid_obese_flag <- data.frame(t(sapply(KWRS_precovid_obese_flag, c)))  %>%
  dplyr::select(p.value, method) %>%
  dplyr::mutate(across(where(is.numeric), round, 3))

p_precovid_obese_flag <- significance_precovid_obese_flag$p.value

median_precovid_obese_flag <- N_median_precovid_obese_flag %>%
  dplyr::left_join(n_median_precovid_obese_flag, by = "precovid_obese_flag") %>%
  dplyr::mutate("N" = N_population - N_missing) %>%
  dplyr::left_join(quartile1_median_precovid_obese_flag, by = "precovid_obese_flag") %>%
  dplyr::left_join(median_median_precovid_obese_flag, by = "precovid_obese_flag") %>%
  dplyr::left_join(quartile3_median_precovid_obese_flag, by = "precovid_obese_flag") %>%
  dplyr::select(precovid_obese_flag, "N", N_population, "25th", median, "75th") %>%
  dplyr::mutate (p = p_precovid_obese_flag) %>%
  dplyr::mutate (p= (as.numeric(p))) %>%
  dplyr::mutate(across(where(is.numeric), round, 3)) %>%
...
write.csv (median_complete, here::here ("output/data","median_range_bmi_2021.csv"))
