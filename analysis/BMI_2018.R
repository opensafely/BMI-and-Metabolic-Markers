...
BMI_complete_long <- demog_2018 %>%
  dplyr::left_join(bmi_2018_long) %>%
...
BMI_2018_median <- median_bmi %>% 
  left_join(demog_2018) %>% 
  dplyr::mutate("year" = "2018")


### classify as underweight, healthyweight, overweight, obese
BMI_complete_categories <- ungroup(BMI_2018_median)
...
###########################################################################################################
write.csv (BMI_data_checks, here::here ("output/data","BMI_data_checks_2018.csv"))
