...
### label
BMI_2018$ethnic_no_miss[BMI_2018$ethnic_no_miss=="1"]<-"White"
BMI_2018$ethnic_no_miss[BMI_2018$ethnic_no_miss=="2"]<-"Mixed"
BMI_2018$ethnic_no_miss[BMI_2018$ethnic_no_miss=="3"]<-"Asian"
BMI_2018$ethnic_no_miss[BMI_2018$ethnic_no_miss=="4"]<-"Black"
BMI_2018$ethnic_no_miss[BMI_2018$ethnic_no_miss=="5"]<-"Other"
BMI_2018$ethnic_no_miss[BMI_2018$ethnic_no_miss=="0"]<-"Not_recorded"
...
BMI_2018$imd[BMI_2018$imd=="0"]<-"NA"
BMI_2018$imd[BMI_2018$imd=="1"]<-"1 most deprived"
BMI_2018$imd[BMI_2018$imd=="2"]<-"2"
BMI_2018$imd[BMI_2018$imd=="3"]<-"3"
BMI_2018$imd[BMI_2018$imd=="4"]<-"4"
BMI_2018$imd[BMI_2018$imd=="5"]<-"5 least deprived"
...
# had_bmi_imd_m <- glm(had_bmi ~ imd, data=BMI_2018, family=binomial) %>%
...
had_bmi_eth_16 <- glm(had_bmi ~ eth_group_16, data=BMI_2018, family=binomial) %>%
 broom::tidy(exponentiate = TRUE, conf.int = TRUE) %>%        # exponentiate and produce CIs
  dplyr::mutate(across(where(is.numeric), round, digits = 2)) 
...
## Add a year flag
BMI_complete_long <- long_bmi_2018 %>%
  mutate("year"= 2018)
...
## 2018 mediam BMI per patient:  BMI_2018_median



## CREATE FLAGS and LABEL DEMOGRAPHIS

BMI_complete_median <- BMI_2018_median
...
write_feather (BMI_complete_categories_DWMP, here::here ("output/data","BMI_complete_median_2018.feather"))

write_feather (BMI_complete_long, here::here ("output/data","BMI_complete_long_2018.feather"))
