...
BMI_plots <- read_feather (here::here ("output/data", "BMI_all_2020.feather"))
BMI_plots <- BMI_plots %>%
...
median_bmi_plot_all <- ggplot( data = BMI_plots, mapping = aes( x = median_bmi)) + labs(title = "Median recorded BMI distribution of patients in 2020", subtitle = "Data from routine primary care electronic health records") + geom_histogram()
...
bmi_age_T2DM <- ggplot( data = BMI_plots, mapping = aes( x = median_bmi)) +  labs(title = "Median recorded BMI distribution of patients in 2020", subtitle = "Data from routine primary care electronic health records") + geom_histogram() + facet_grid (age_group ~comorbid_diabetes_t2)
bmi_age_hypertension <- ggplot( data = BMI_plots, mapping = aes( x = median_bmi)) + labs(title = "Median recorded BMI distribution of patients in 2020", subtitle = "Data from routine primary care electronic health records") +  geom_histogram() + facet_grid (age_group ~comorbid_hypertension)


ggsave(
    plot = median_bmi_plot_all,
    filename = "median_bmi_all_2020.png", 
...
ggsave(
    plot = bmi_age_T2DM,
    filename = "median_bmi_age_T2DM_2020.png", 
    path = here::here("output"),
...
ggsave(
    ...
    filename = "median_bmi_age_hypertension_2020.png", 
    path = here::here("output"),
    dpi=600, width = 30, height = 30, units = "cm"
)
